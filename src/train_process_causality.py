...
def evaluate_causal_word(args, model, criterion, test_generator, count_limit=None, global_batch=0):
    ...
    if True:
        ...
        for iter, batch_data0 in enumerate(test_generator):
            ...
            if args.saliancy_method == 'compute_saliancy_batch_grad':
                loss_gradspred = compute_saliancy(
                    args, model, batch_data, retain_graph=False)
                # print(loss_gradspred)
                # exit()
    ...
    for epoch in range(args.epoch):
        ...
        with torch.autograd.set_detect_anomaly(True):
            for iter, batch_data0 in enumerate(train_generator):
                ...
                duration = int(len(train_generator)/5)
                if iter % duration == 0 or iter == len(train_generator) - 1:
                    ...
                    val_accuracy, f1, val_loss, eval_ratio = evaluate_causal_word(
                        args, model, criterion, test_generator, count_limit=15, global_batch=global_batch)
