...
parser.add_argument('--causal_ratio', type=float, default=-0.001, help='batch_size')
parser.add_argument('--batch_size_test', type=int, default=None, help='batch_size_test')
parser.add_argument('--epoch', type=int, default=None, help='epoch')
parser.add_argument('--dataset', type=str, default='mini-SNLI', help='dataset')
parser.add_argument('--grad_loss_func', type=str, default='argmax_loss', help='grad_loss_func')
parser.add_argument('--saliancy_method', type=str, default='compute_saliancy_batch', help='saliancy_method')
parser.add_argument('--train_process', type=str, default='train', help='train_process')
parser.add_argument('--model_name_or_path', type=str, default='bert-base-uncased', help='model_name_or_path')
parser.add_argument('--databunch_method', type=str, default='DataBunch', help='databunch_method')
parser.add_argument('--use_custom_bert', action='store_true', default=False, help='use_custom_bert')
#parser.add_argument('--tokenizer_name', type=str, default='bert-base-uncased', help='model_name_or_path')
parser.add_argument('--load_few', action='store_true', default=False, help='load few')
parser.add_argument('--grad_clamp', action='store_true', default=False, help='grad_clamp')
parser.add_argument('--test_mode', action='store_true', default=False, help='test_mode or not')
...
if __name__ == "__main__":
    ...
    if config.do_train:
        train_generator = data.DataLoader(trainset[config.dataset_train], **params_trainset)
        dev_generator = data.DataLoader(devset[config.dataset_train],**params_trainset)
    ...
    if config.do_test:
        model.load_state_dict(torch.load(config.model_save_path))
        test_generator = data.DataLoader(testset[config.dataset_test], **params_testset)
